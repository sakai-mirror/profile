...
public class ProfileTool
{
	...
	public String processActionEditSave()
	{
		...
		else if ((getPictureIdPreference() != null) && (getPictureIdPreference().equals(UPLOAD)))
		{
			
			if(fileAsStream == null) {

				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(rb.getString("error_no_file_supplied")));
			...
			profile.setInstitutionalPictureIdPreferred(new Boolean(false));
			displayPicture = true;
			this.pictureIdPreference = UPLOAD;
			...
			if(fileSize > maxPictureSize*1024) {
				...
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(rb.getString("error_file_too_large") + " (max " + maxPictureSize + " kbytes): " + filename));
			...
			{
				LOG.warn("Filetype was unknown: " + fileContentType);
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(rb.getString("error_filetype_unknown") + fileContentType));
				return "edit";
			}
			...
			try {
				...
				LOG.error("IOException while attempting the upload of file " + filename + " during processAdd "
						+ ". " + e.getMessage());
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(rb.getString("error_exception_upload")));
				
				return "edit";
			}

			try {
				String url = profileService.setPhoto(fileContents, fileContentType);
				profile.setPictureUrl(url);
			} catch (Exception e) {
				LOG.error("Exception caught while setting profile photo: " + e + " " + e.getMessage());
   ...
   public boolean isShowSearch()
   {
	...
	throws AbortProcessingException {
		...
		LOG.debug("processFileUpload() event: " + event
		...
		LOG.debug("processFileUpload(): item: " + item
